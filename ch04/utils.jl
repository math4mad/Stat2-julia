...
function plot_cor_group(data::Union{SubDataFrame,AbstractDataFrame})
        ...
        fig=Figure(resolution=(1100,row*300))
...
"""
    plot_fitline_residuals(data::AbstractDataFrame, model::RegressionModel;feature:: Union{Vector{AbstractString},Vector{Symbol}})
    绘制拟合线和残差图
"""
function plot_fitline_residuals(data::AbstractDataFrame, model::RegressionModel;feature::Union{Vector{String},Vector{Symbol}})
       
    xs=select(data,feature[1])
    yhat=predict(model,xs)
    res=residuals(model)
    fig=Figure(resolution=(900,400))
    ax1=Axis(fig[1,1];xlabel=feature[1],ylabel=feature[2])
    ax2=Axis(fig[1,2];xlabel="Predict Price",ylabel="Residuals")
    scatter!(ax1,data[:,feature[1]],data[:,feature[2]]
    ;marker=:circle,markersize=12,color=(:purple,0.2),strokewidth=1,strokecolor=:black,
    )
    lines!(ax1,data[:,feature[1]],yhat,label="fitline",linewidth=3,color=:red)
    scatter!(ax2,yhat,res
    ;marker=:circle,markersize=12,color=(:purple,0.2),strokewidth=1,strokecolor=:black,
    )
    hlines!(ax2,[0],linewidth=2, linestyle=:dot,color=:red)
    axislegend(ax1)
    fig
 end