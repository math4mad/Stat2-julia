...
def _(data, dataset_name, eda, message, mo):
    if data is None:
        _output = mo.vstack([mo.md("# UnitC-LogisticRegression/ch09/p1106-sleeptime-child.jl\n\n**Migration:** logistic/generalized linear model."), mo.md(message)])
    else:
        _output = mo.vstack([mo.md("# UnitC-LogisticRegression/ch09/p1106-sleeptime-child.jl\n\n**Migration:** logistic/generalized linear model."), mo.md(f"Dataset: `{dataset_name}`"), data.head(), eda])
    _output
    return
...
@app.cell
def _(data, mo):
    model_formula = 'more7proprotion ~Age'
    if data is None or model_formula is None:
        _output = mo.md("Model translation remains TODO for this source note.")
    else:
        _output = mo.md(f"Detected Julia formula: `{model_formula}`. Classified as **logistic/generalized linear model**; translate with statsmodels or scipy.")
    _output
    return
